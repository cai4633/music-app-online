...
//工具函数
const insert = (playlist: any[], sequencelist: any[], currentIndex: number, song: Songs) => {
...
export const insertSong = ({ commit, state }: any, song: Songs) => {
  const { plist, slist } = insert(state.playlist, state.sequencelist, state.currentIndex, song)
  let index = 0
  if (state.currentIndex !== -1) {
    index = findIndex(plist, state.playlist[state.currentIndex])
  }
...
// 我的喜欢
export const saveFavoriteList = ({ commit, state }: any, song: Songs) => {
  commit(types.SET_FAVORITE, Favorite.saveFavorite(song))
}
export const deleteFromFavoriteList = ({ commit, state }: any, song: Songs) => {
  commit(types.SET_FAVORITE, Favorite.removeFavorite(song))
}
